...
router.beforeEach((to, from, next) => {
    ...
    if (Cookies.get('locking') === '1' && to.name !== 'locking') { // 判断当前是否是锁定状态
        ...
        if (!Cookies.get('user') && to.name !== 'login' && !Cookies.get('routeName')) { // 判断是否已经登录且前往的页面不是登录页
            ...
            if (to.name !== 'home_index') {
                (async () => {
                    try {
                        await post('right/users', {navUrl: to.name}).then(res => {
                            let content = res.data;
                            if (content.status === 200) {
                                store.commit('UPDATE_MENU_RIGHTS', content.res);
                            }
                        });
                    }catch (e) {
                    }
