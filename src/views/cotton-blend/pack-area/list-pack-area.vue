    ...
    <Card>
        <Row type="flex" justify="space-between">
            ...
            <Col class="flex-left">
                ...
                <Select clearable v-model="queryBarTypeId" placeholder="请选择抓包方式" class="queryBarMarginRight searchHurdles">
                ...
                <Input v-model="searchValue" placeholder="请输入编号或名称" class="queryBarMarginRight searchHurdles"/>
    ...
    export default {
        ...
        methods: {
            ...
            deleteModalConfirm () {
                ...
                this.$call('packing.area.delete', this.validatorState(1).ids).then(res => {
                    if (res.data.status === 200) {
            ...
            getTypeListRequest (resolve, reject) {
                return this.$call('dict.list', {parentCode: 'grabbing_type'}).then(res => {
                    if (res.data.status === 200) {
                        let responseData = res.data.res;
                        resolve(res);
            ...
            getProcessListRequest (resolve, reject) {
                return this.$api.process.getSearchProcessList().then(res => { this.processList = res; resolve(res); });
            }
        },
        created () {
            let workshopList = new Promise((resolve, reject) => this.getWorkshopHttp(resolve, reject));
            let typeList = new Promise((resolve, reject) => this.getTypeListRequest(resolve, reject));
            let auditStateList = new Promise((resolve, reject) => this.getAuditStateListRequest(resolve, reject));
            let processList = new Promise((resolve, reject) => this.getProcessListRequest(resolve, reject));
            Promise.all([workshopList, typeList, auditStateList, processList]).then(res => {
