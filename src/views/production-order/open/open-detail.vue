<template>
    <div>
        ...
        >
            <div slot="content">
                <Form :label-width="98" ref="openMachineDetail" :model="openMachineDetail" :rules="openMachineValidate" :show-message="false">
                    <Row>
                        <Col span="8">
                        ...
                        <Col span="8">
                            <FormItem label="班次日期：" prop="startBelongDate" class="formItemMargin">
                                <p class="modal-readonly">{{ openMachineDetail.startBelongDate }}</p>
                            </FormItem>
                        </Col>
                        <Col span="8">
                            <FormItem label="开台班次：" prop="startShiftName" class="formItemMargin">
...
export default {
    ...
    watch: {
        openMachineDetailId (newData, oldData) {
            const _this = this;
            if (newData) {
                this.$call('prd.notice.machine.opening.detail3', {id: newData}).then(res => {
                    let content = res.data;
                    if (content.status === 200) {
                        _this.openMachineDetail = content.res;
                        _this.openMachineDetail.startScheduleId = content.res.startScheduleId;
                        _this.openMachineDetail.curTime = curDatetime();
                        _this.openMachineDetail.newStartSpinNumber = this.openMachineDetail.startSpinNumber;
                        _this.openMachineDetail.newEndSpinNumber = this.openMachineDetail.endSpinNumber;
                        _this.newOpenSpinCount = this.openMachineDetail.openSpinCount;
                        this.getScheduleTime();
                        _this.show = true;
                    }
                });
