    ...
    <div slot="content">
      ...
      >
        <div slot="content">
          ...
          <Row class="pageHeight">
            <Col class="pageStyle">
              <Page :total="modalTotal" size="small" />
            </Col>
          </Row>
        </div>
      </yf-modal>
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.showModal = !this.showModal //showwww
                    this.getModalList()
                  }
                }
              }, '创建DOM')
  ...
  methods: {
    ...
    getModalList () {
      return this.$call('bom.list', {
        auditState: 3,
        productId: this.tableData.id
      }).then((res) => {
        if (res.data.status === 200) {
          // console.log(res.data, 'getModalList');
          this.modalTableData = res.data.res
          this.modalTotal = res.data.length - 1
        }
      })
    },
    // 获取id
