    ...
    <div>
        ...
        >
            <div slot="content">
                        ...
                        <Col class="">
                            <Row class="marginBottom">
                                <Col>
                                    ...
                                    <span
                                            class=""
                                            style="text-align: center;margin-bottom: 10px; ;line-height: 32px; "
                                    >-</span
                                    >
                                    <DatePicker
                                            class="formEachStyle "
                            ...
                            <Row v-if="orderSelectedShow">
                                <Col>
                                    ...
                                    <Input
                                            ...
                                            class="formWidth"
                                            type="text"
                                            v-model="prdOrderNoticeCodeName"
                                            placeholder="请输入编码"
                                    />
                                    <Select
                                            class="formEachStyle textLeft"
                                    ...
                                    <Select
                                            class="formEachStyle textLeft"
                                            v-show="curCompletionState === 0"
                                            v-model="replacementState"
                                            placeholder="请选择配件更换状态"
                                    >
                                        <Option
                                                v-for="(item, index) in replacementStateList"
                                                :value="item.id"
                                                :key="item.id"
                                        >{{ item.name }}
                                        </Option
                ...
                <Table
                        @on-selection-change="onSelectionEvent"
                        class="marginBottom"
                        :height="tableHeight"
                        :loading="processModificationLoading"
                        size="small"
                        ref="selection"
                        border
                        :columns="processModificationColumns"
                        :data="processModificationData" processModificationData
                ></Table>
    ...
    <!-- 设备工艺或运转工艺-->
        ...
        >
            <div slot="content">
                <Form
                        :label-width="90"
                        ref="processModificationForm"
                        :rules="processModificationFormRule"
                        :model="processModificationForm"
                        :show-message="false"
                >
                    <Row>
                        <Col span="8">
                            <FormItem label="设备机型:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.machineModelName }}
                                </div>
                            </FormItem>
                        </Col>
                        ...
                        <Col span="8">
                            <FormItem label="标准克重:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.gramWeight }}
                                </div>
                            </FormItem>
                        </Col>
                        ...
                        <Col span="8">
                            <FormItem label="标准米长:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.meters }}
                                </div>
                            </FormItem>
                        </Col>
                        <Col span="8">
                            ...
                            <FormItem label="台时单产:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.hourYield }}
                                </div>
                            </FormItem>
                        </Col>
                        <Col span="8">
                            <FormItem label="公定回潮率%:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.moistureRegain }}
                                </div>
                            </FormItem>
                        </Col>
                        <Col span="8">
                            <FormItem label="运转效率%:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.efficiencyPercent }}
                                </div>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row v-if="!isChangeSpecSet">
                        <Col span="8">
                            ...
                            >
                                <Select
                                        class="formSWidth"
                                        @on-change="changeTubeType"
                                        clearable
                                        v-model="processModificationForm.tubeTypeId"
                                >
                                    <Option
                                            v-for="item in tubeType"
                                            :value="item.id"
                                            :key="item.id"
                                    >{{ item.name }}
                                    </Option
                    ...
                    <Row>
                        <Col span="8">
                            <FormItem label="上机产品:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.productName }}
                                </div>
                        ...
                        <Col span="8">
                            <FormItem label="改前产品:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ processModificationForm.oldProductName }}
                                </div>
                                <!--<Select class="formSWidth" v-model="processModificationForm.tubeColorIds" multiple>-->
                                <!--<Option v-for="item in tubeColor" :value="item.id" :key="item.id">{{ item.name }}</Option>-->
                                <!--</Select>-->
                ...
                <Table
                        height="500"
                        :loading="isProcessModificationModalLoading"
                        size="small"
                        ref="selection"
                        border
                        :columns="isProcessModificationColumns"
                        :data="isProcessModificationData"
                ></Table>
            </div>
        </modal>
        ...
        <modal
                ...
                :loading="isProcessModificationLoading"
                :width="1000"
                :title="isFlipProcessTitle"
                @cancel="flipProcessTitleCancel"
                @submit="flipProcessTitleSubmit"
        >
            <div slot="content">
                <Form
                        :label-width="90"
                        ref="processModificationForm"
                        :model="flipProcessTitleForm"
                        :show-message="false"
                >
                <!--   :rules="flipProcessTitleFormRule" -->
                    <Row>
                        <Col span="8">
                            <FormItem label="设备机型:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ flipProcessTitleForm.machineModelName }}
                                </div>
                            </FormItem>
                        </Col>
                        ...
                        <Col span="8">
                            <FormItem label="标准克重:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ flipProcessTitleForm.gramWeight }}
                                </div>
                            </FormItem>
                        </Col>
                        ...
                        <Col span="8">
                            ...
                            <FormItem label="台时单产:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ flipProcessTitleForm.hourYield }}
                                </div>
                            </FormItem>
                        </Col>
                        <Col span="8">
                            <FormItem label="公定回潮率%:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ flipProcessTitleForm.moistureRegain }}
                                </div>
                            </FormItem>
                        </Col>
                        <Col span="8">
                            <FormItem label="运转效率%:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ flipProcessTitleForm.efficiencyPercent }}
                                </div>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row v-show="isflipProcesSelsct">
                        <Col span="8">
                            <FormItem
                                    label="管圈类型:"
                                    class="formItemMargin"
                            ...
                            >
                                <Select
                                        ...
                                        @on-change="changeTubeType"
                                        clearable
                                        v-model="flipProcessTitleForm.tubeTypeId"
                                >
                                    <Option
                                            v-for="item in tubeType"
                                            :value="item.id"
                        ...
                        <Col span="16">
                            ...
                            >
                                ...
                                <!--</Select>-->
                                <Select
                                        v-model="flipProcessTitleForm.tubeColorIds"
                                        multiple
                                        filterable
                                        remote
                                        :remote-method="remoteMethod2"
                    ...
                    <Row>
                        <Col span="8">
                            <FormItem label="上机产品:" class="formItemMargin">
                                <div class="modal-readonly">
                                    {{ flipProcessTitleForm.productName }}
                                </div>
                                <!--<Select class="formSWidth" v-model="processModificationForm.tubeTypeId">-->
                                <!--<Option v-for="item in tubeType" :value="item.id" :key="item.id">{{ item.name }}</Option>-->
                                <!--</Select>-->
