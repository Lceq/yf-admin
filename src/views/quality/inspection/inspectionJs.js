...
export default ({
    ...
    methods: {
        ...
        addNewQmSearch () {
            ...
            this.testFormValidate.prdNoticeId = '';
        ...
        deleteQmWeight () {
            if (this.curQmSearchValue.length!==this.curQmSearchValue.length) {
                this.$Modal.warning({
                    title: '提示',
        ...
        deleteWarnConfirm () {
            ...
            // this.$api.inspection.deleteQmWeight(ids).then(res => {
            //     console.log(res)
            //     let content = res.data;
            //     this.deleteWarnLoading = false;
            //     if (content.status === 200) {
    ...
    created () {
        ...
        this.$api.dictionary.getQmType().then(res => {
            ...
            if (content.status === 200) {
                ...
                this.dataType = content.res.find(x => x.name === '条干检验');
