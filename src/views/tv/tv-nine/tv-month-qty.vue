    ...
    <div id="tv" :style="'width: ' + width + 'px; height: ' + height + 'px;display: inline-block;'">
        <div class="parentFlexBetween  verticalMiddle">
            <div class="verticalMiddle">
                <img style="height:30px;position: relative;" class="verticalMiddle" src="../../../images/zg.png"  alt="正凯" />
                <Select v-model="workshopId" class="selectBackground verticalMiddle" style="width: 100px; " placeholder="请选择车间">
                    <Option v-for="(item, index) in workshopList" :value="item.deptId" :key="item.deptId">{{ item.deptName }}</Option>
                </Select>
                <Button class="" type="primary" shape="circle" size="small" @click="expandCharts" :icon="!value ? 'ios-expand' : 'ios-exit'"></Button>
            </div>
